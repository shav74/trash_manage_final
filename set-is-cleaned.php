...
if (isset($_POST['submit'])) {
  ...
  $cleaning_status = mysqli_real_escape_string($connection, $_POST['cleaning-status']);
  // prepare database query
  $query = "UPDATE report SET cleaning_progress='$cleaning_status' WHERE id=$report_id";

  $result_set = mysqli_query($connection, $query);

  verify_query($result_set);

  header('Location: cleaning-list.php');
...
<body>
  ...
  <form action="set-is-cleaned.php?id=<?= $report_id ?>" method="post">
    ...
    <fieldset>
      ...
      <p>
        <label for="cleaning-status">Cleaning Progress: </label>
        <select name="cleaning-status" id="">
          <option disabled selected> Select an option</option>
          <option value="1">Yet to start</option>
          <option value="2">Cleaning in progress</option>
